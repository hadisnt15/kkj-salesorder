<x-layout>
                            ...
                            <div class="flex items-center">
                                <div class="flex-1 min-w-0 ms-4">
                                    <p class="text-sm font-medium truncate text-white">
                                        {{ $emp->EmpCode }} | {{ strtoupper($emp->user->name) }}
                                    </p>
                                    <p class="text-sm font-sm truncate text-white capitalize">
                                        {{ strtoupper($emp->user->level) }} | {{ $emp->div }}
                                    </p>
                                    <button data-modal-target="user-view" data-modal-toggle="user-view" type="button"
                                        class="open-modal-user-btn" data-name="{{ strtoupper($emp->user->name) }}"
                                        data-email="{{ $emp->user->email }}"
                                        data-level="{{ strtoupper($emp->user->level) }}"
                                        data-created_at="{{ $emp->user->created_at }}" data-div="{{ $emp->div }}">
                                        <a href="#">
                                            <span
                                    ...
                                    <button type="button">
                                        <a href="{{ route('user.edit', $emp->id) }}" class="">
                                            <span
                                    ...
                                    <button type="button">
                                        <a href="{{ route('user.delete', $emp->id) }}" class="">
                                            <span
                        ...
                        <tr
                            ...
                            <td class="px-6 py-4">
                                <button data-modal-target="user-view" data-modal-toggle="user-view" type="button"
                                    ...
                                    data-name="{{ strtoupper($emp->user->name) }}"
                                    data-email="{{ $emp->user->email }}"
                                    data-level="{{ strtoupper($emp->user->level) }}"
                                    data-created_at="{{ $emp->user->created_at }}" data-div="{{ $emp->div }}">
                                    <a href="#">
            ...
            <div class="relative rounded-lg shadow-sm bg-red-600 opacity-75">
                ...
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-gray-600 ">
                    <h3 class="text-xl font-medium text-white">
                        ...
                        <span id="modallevel" class="capitalize text-sm"></span> | <span id="modaldiv"
                            class="capitalize text-sm"></span>
                    </h3>
                    <button type="button"
    ...
    <script>
        // Fungsi buka modal
        function openModalView(name, email, level, created_at, div) {
            ...
            document.getElementById('modaldiv').innerText = div;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            buttonsView.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    openModalView(name, email, level, created_at, div);
                });
            });
        });
    </script>
