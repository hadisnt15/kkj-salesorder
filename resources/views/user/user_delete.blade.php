<x-layout>
    ...
    <form class="mx-auto " action="{{ route('user.destroy', $usr->id) }}" method="post"> @csrf @method('delete')
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-sm font-medium text-white"><i
                                class="ri-delete-back-2-fill me-1"></i>Delete User</span>
                    </div>
                </li>
            </ol>
        </nav>
        <div class="opacity-75 border border-y-zinc-800 rounded-lg sm:max-w-xl mx-auto bg-red-600">
            <div class="border-b ">
                <div class="">
                    <h3 class="text-xl font-medium text-white pt-4 px-3">
                        {{ $usr->name }} <br>
                    </h3>
                    <p class="text-sm font-medium text-white py-2 px-3">
                        {{ strtoupper($usr->level) }} - {{ ($usr->status) == 1 ? 'ACTIVE' : 'NON ACTIVE' }}
                    </p>
                </div>
            </div>
            <div class="border-b border-gray-500">
                <div class="bg-red-300">
                    <div class="py-4">
                        <p class="py-1 text-sm font-medium text-black px-8">Email:
                            {{ $usr->phone }}</p>
            ...
            <div class="border-b border-gray-500">
                <div class="bg-red-300">
                    <div class="py-4">
                        <p class="py-1 text-sm font-medium text-black px-8">Joined Since:
                            {{ $usr->created_at }}</p>
                    </div>
                </div>
            </div>
